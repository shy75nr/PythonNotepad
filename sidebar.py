...
def get_lineno(text, index):
    ...
    return int(float(text_index))+int(len(text.get(1.0, 'end'))/160) if text_index else None
...
class BaseSideBar:
    ...
    def __init__(self, editwin):
        ...
        # self.update_font(idleConf.GetFont(self.text, 'main', 'EditorWindow'))
        self.update_font(("Microsoft YaHei UI",10,"bold"))
        self.update_colors()
    ...
    #     self.main_widget.yview_scroll(int(-1 * (event.delta / 120)), "units")
    #     self.text.yview_scroll(self.editwin.vbar.get())
    #     return 'break'
...
class LineNumbers(BaseSideBar):
    ...
    def init_widgets(self):
        ...
        self.sidebar_text = tk.Text(self.parent, width=1, wrap=tk.NONE,
                                    padx=2, pady=pady,
                                    borderwidth=0, highlightthickness=0, spacing1=0)
    ...
    def grid(self):
        ...
        self.sidebar_text.pack(fill=tk.Y)

    def update_font(self, font):
        # font = idleConf.GetFont(self.text, 'main', 'EditorWindow')
        self.sidebar_text['font'] = font

    def update_colors(self,fg="SteelBlue"):
        """Update the sidebar text colors, usually after config changes."""
    ...
    def yscroll_event(self, *args, **kwargs):
        ...
        # self.main_widget.yview_scroll(int(-1*eval(args[0])/120),'units')
        return 'break'
